...
public class LabController {
    ...
    public LabController() {
        ...
        choicesHandles.put(
                PUSH, (args) -> {
                    ...
                    } else return "error: number must be positive";
                }
        );
        choicesHandles.put(
                GENERATE_GREATER, (args) -> {
                    StringBuilder buildResult = new StringBuilder();
                    buildResult.append("[");
                    listToGenerate.stream().map(x -> Math.abs(random.nextInt()) + x).
                            forEach(x -> buildResult.append(x.toString() + ", "));
                    buildResult.append("]");
        ...
        choicesHandles.put(
                GENERATE_LESS, (args) -> {
                    StringBuilder buildResult = new StringBuilder();
                    buildResult.append("[");
                    listToGenerate.stream().map(random::nextInt)
                            .forEach(x -> buildResult.append(x.toString() + ", "));
                    buildResult.append("]");
                    return buildResult.toString();
                }
        ...
        choicesHandles.put(
                MIDDLE, (args) -> {
                    return String.valueOf((float)listToGenerate.stream().
                                    reduce(0, (akk, value) -> akk + value) / listToGenerate.size());
                }
        );
    }
    ...
    @PostMapping("/")
    @ResponseBody
    public String resolveChoice(@RequestParam("submit") String typeOfGeneration,
